...
export default function GroupScreen() {
  ...
  const [activeTab, setActiveTab] = React.useState<"expenses" | "activity">(
    "expenses",
  );
  ...
  const [isFundBreakdownModalVisible, setIsFundBreakdownModalVisible] =
    React.useState(false);
  const [contributeAmountStr, setContributeAmountStr] = React.useState("");
  const [contributePayerId, setContributePayerId] = React.useState<
    string | null
  >(null);
  const [contributeAccountId, setContributeAccountId] = React.useState<
    string | null
  >(null);
    ...
    React.useCallback(() => {
      const fetchData = async () => {
        ...
        const sData = await db
          ...
          .where(eq(groupSettlements.groupId, id));
  ...
  const { humanBalances, fundBalances } = React.useMemo(() => {
    if (!members || !allParticipants)
      return { humanBalances: {}, fundBalances: {} };
    ...
    allParticipants.forEach((p) => {
      ...
      if (fundMember && payerId === fundMember.id) {
        if (fBals[p.memberId] !== undefined)
          fBals[p.memberId] += p.paidShare - p.owedShare;
      } else {
        if (hBals[p.memberId] !== undefined)
          hBals[p.memberId] += p.paidShare - p.owedShare;
      }
    });
    ...
    settlements.forEach((s) => {
      if (
        fundMember &&
        (s.toMemberId === fundMember.id || s.fromMemberId === fundMember.id)
      ) {
        if (fBals[s.fromMemberId] !== undefined)
          fBals[s.fromMemberId] += s.amount;
        if (fBals[s.toMemberId] !== undefined) fBals[s.toMemberId] -= s.amount;
      } else {
        if (hBals[s.fromMemberId] !== undefined)
          hBals[s.fromMemberId] += s.amount;
  ...
  const suggestedRepayments = React.useMemo(() => {
    ...
    return instructions.filter(
      (i) => i.from !== fundMember.id && i.to !== fundMember.id,
    );
  ...
  const actualRepayments = React.useMemo(() => {
    ...
    expenses.forEach((e) =>
      events.push({ date: e.date, type: "expense", data: e }),
    );
    settlements.forEach((s) =>
      events.push({ date: s.date, type: "settlement", data: s }),
    );
    ...
    events.forEach((ev) => {
      if (ev.type === "expense") {
        ...
        const s = ev.data;
        if (runningBals[s.fromMemberId] !== undefined)
          runningBals[s.fromMemberId] += s.amount;
        if (runningBals[s.toMemberId] !== undefined)
          runningBals[s.toMemberId] -= s.amount;
      }

      // Check if settled
      const isSettled = Object.values(runningBals).every(
        (b) => Math.abs(b) < 0.01,
      );
      if (isSettled) {
        ...
        if (ower.owedShare > 0) {
          parts.forEach((payer) => {
            if (payer.paidShare > 0 && ower.memberId !== payer.memberId) {
              const amountOwedToPayer =
                ower.owedShare * (payer.paidShare / totalPaid);
    ...
    activeSettlements.forEach((s) => {
      if (
        pairwise[s.fromMemberId] &&
        pairwise[s.fromMemberId][s.toMemberId] !== undefined
      ) {
  ...
  const activityLog = React.useMemo(() => {
    ...
    expenses.forEach((e) => {
      logs.push({
        ...
        type: "expense",
        date: e.date,
        text: `Added expense: ${e.description}`,
      });
    });

    settlements.forEach((s) => {
      const fromName =
        members.find((m) => m.id === s.fromMemberId)?.name || "Someone";
      const toName =
        members.find((m) => m.id === s.toMemberId)?.name || "Someone";
      logs.push({
        id: `set-${s.id}`,
        type: "settlement",
        date: s.date,
        text: `${fromName} paid ${toName} $${s.amount.toFixed(2)}`,
      });
    });
    ...
    try {
      ...
      if (payer?.isUser && contributeAccountId) {
        const tripCat =
          useStore
            .getState()
            .expenseCategories.find((c) => c.name === "Group Contribution") ||
          useStore
            .getState()
    ...
    try {
      ...
      if (expenses.length > 0) {
        ...
        expenses.forEach((e) => {
          ...
          const payerNames = payers
            .map(
              (p) =>
                members.find((m) => m.id === p.memberId)?.name || "Someone",
            )
            .join(", ");
          ...
          const title =
            e.description && e.description.toLowerCase() !== "group expense"
      ...
      members
        ...
        .forEach((m) => {
          ...
          if (bal > 0.01)
            report += `- ${m.name}: +$${bal.toFixed(2)} (Gets back)\n`;
          else if (bal < -0.01)
            report += `- ${m.name}: -$${Math.abs(bal).toFixed(2)} (Owes)\n`;
      ...
      report += `\nGenerated by Slicent Expense Tracker`;
  ...
  const handleArchiveToggle = async () => {
    ...
    if (!group.isArchived) {
      const hasDebts =
        Object.values(humanBalances).some((b) => Math.abs(b) > 0.01) ||
        Object.values(fundBalances).some((b) => Math.abs(b) > 0.01);
      if (hasDebts) {
        Alert.alert(
          "Cannot Archive",
          "All debts must be settled before archiving this group.",
        );
        return;
      }
    }
    ...
    try {
      await db
        .update(groups)
        .set({ isArchived: !group.isArchived })
        .where(eq(groups.id, group.id));
      setGroup({ ...group, isArchived: !group.isArchived });
      Alert.alert(
        "Success",
        `Group ${group.isArchived ? "unarchived" : "archived"} successfully.`,
      );
    } catch (error) {
  ...
  const renderExpense = ({
    ...
    item: typeof groupExpenses.$inferSelect;
  }) => {
      ...
      >
        ...
        <View style={styles.expenseInfo}>
          <Text style={styles.expenseDesc}>
            {item.description || cat?.name || "Group Expense"}
          </Text>
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={["top", "bottom"]}>
          ...
          <View>
            ...
            <View style={styles.sectionHeaderWrap}>
              ...
              <TouchableOpacity
                style={[
                  styles.settleUpBtn,
                  expenses.length === 0 && { opacity: 0.5 },
                ]}
                onPress={() =>
                  router.push(`/groups/${group.id}/settle-up` as any)
                }
                  ...
                  return (
                    <View key={m.id} style={styles.balanceRow}>
                      <View style={styles.memberAvatar}>
                        <Ionicons
                          name={m.isUser ? "person" : "person-outline"}
                          size={16}
                          color="#6366F1"
                        />
                      </View>
                      ...
                      <Text
                        style={[styles.memberBalance, { color: statusColor }]}
                      >
                        {statusText}
                      </Text>
                    </View>
                        ...
                        <View key={idx} style={styles.repaymentRow}>
                          <Text style={styles.repaymentText}>
                            <Text style={{ fontWeight: "700" }}>
                              {fromMember?.isUser ? "You" : fromMember?.name}
                            </Text>
                            {" pay "}
                            <Text style={{ fontWeight: "700" }}>
                              {toMember?.isUser ? "You" : toMember?.name}
                            </Text>
                          </Text>
            ...
            {actualRepayments.length > 0 && (
              <View style={{ marginTop: 16 }}>
                ...
                >
                  <Text style={styles.sectionTitle}>
                    Actual Repayments (Exact)
                  </Text>
                  <Ionicons
                    name={isActualExpanded ? "chevron-up" : "chevron-down"}
                    size={20}
                    color={Colors.light.textSecondary}
                  />
                </TouchableOpacity>
            ...
            <View style={styles.tabToggle}>
              <TouchableOpacity
                style={[
                  styles.tabBtn,
                  activeTab === "expenses" && styles.tabBtnActive,
                ]}
        ...
        renderItem={({ item }) => {
          ...
          return (
            <View style={styles.activityCard}>
              <View
                style={[styles.activityIcon, { backgroundColor: color + "15" }]}
              >
          ...
          activeTab === "activity" ? (
            <View style={styles.footerSection}>
              <TouchableOpacity
                style={[
                  styles.archiveBtn,
                  group.isArchived && styles.unarchiveBtn,
                ]}
                onPress={handleArchiveToggle}
              >
                <Ionicons
                  name={
                    group.isArchived ? "folder-open-outline" : "archive-outline"
                  }
                  size={20}
                  color={group.isArchived ? "#6366F1" : "#EF4444"}
                />
                <Text
                  style={[
                    styles.archiveBtnText,
                    group.isArchived && { color: "#6366F1" },
                  ]}
                >
              ...
              <Text style={styles.archiveHelp}>
                {group.isArchived
                  ...
                  : "If the group trip is over you can archive it, but all balances must be settled first."}
              </Text>
            </View>
          ) : (
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: "#FFF" }}>
          <View style={styles.modalHeader}>
            <TouchableOpacity onPress={() => setSelectedExpense(null)}>
              <Text
                style={{ color: "#6366F1", fontSize: 16, fontWeight: "600" }}
              >
                Close
              </Text>
            </TouchableOpacity>
                  ...
                  {(() => {
                    ...
                    const isFundExpense =
                      ...
                      allParticipants.some(
                        (p) =>
                          p.expenseId === selectedExpense.id &&
                          p.memberId === fundMember.id &&
                          p.paidShare > 0,
                      );
                    ...
                    return members.map((m) => {
                      ...
                      if (
                        !participation ||
                        (participation.paidShare === 0 &&
                          participation.owedShare === 0)
                      )
                        return null;
                      ...
                      let label =
                        participation.paidShare > 0 ? "Share:" : "Owed:";
                          ...
                          <View style={{ alignItems: "flex-end" }}>
                            {participation.paidShare > 0 && (
                              <Text style={styles.breakdownPaid}>
                                Paid: ${participation.paidShare.toFixed(2)}
                              </Text>
                            )}
                            {participation.owedShare > 0 && (
                              <Text style={styles.breakdownOwed}>
                                {label} ${participation.owedShare.toFixed(2)}
                              </Text>
                            )}
                          </View>
                        </View>
      ...
      >
        ...
        >
          ...
          <ScrollView style={styles.modalContent}>
              ...
              {members
                ...
                .map((m) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.memberChip,
                      contributePayerId === m.id && styles.memberChipActive,
                    ]}
                  ...
                  {userAccounts.map((acc) => (
                    ...
                    >
                      <Ionicons
                        ...
                        size={16}
                        color={
                          contributeAccountId === acc.id ? "#FFF" : acc.color
                        }
                      />
                      <Text
                        style={[
            ...
            >
              <Text
                style={[styles.saveBtnText, { color: "#FFF", fontSize: 16 }]}
              >
                Add to Group Bank
              </Text>
            </TouchableOpacity>
      ...
      <Modal
        visible={isFundBreakdownModalVisible}
        animationType="slide"
        presentationStyle="pageSheet"
        onRequestClose={() => setIsFundBreakdownModalVisible(false)}
      >
        <SafeAreaView
          style={{ flex: 1, backgroundColor: Colors.light.background }}
        >
          <View style={styles.modalHeader}>
            ...
            <TouchableOpacity
              onPress={() => setIsFundBreakdownModalVisible(false)}
            >
              <Ionicons
                name="close"
                size={24}
          ...
          <ScrollView style={styles.modalContent}>
            <View style={{ alignItems: "center", marginBottom: 32 }}>
              ...
              <Text
                style={{ fontSize: 36, fontWeight: "800", color: "#10B981" }}
              >
                $
                {Math.abs(
                  fundBalances[members?.find((m) => m.isFund)?.id || ""] || 0,
                ).toFixed(2)}
              </Text>
            </View>
                    ...
                    <View key={m.id} style={styles.breakdownRow}>
                      ...
                      <View style={{ alignItems: "flex-end" }}>
                        <Text
                          style={[
                            getsBack
                              ? styles.breakdownPaid
                              : styles.breakdownOwed,
                            { fontSize: 15 },
                          ]}
                        >
                          {getsBack ? "Gets back" : "Owes fund"}: $
